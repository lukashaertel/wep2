...
data class After<K : Comparable<K>>(val key: K) : Query<K>() {
    ...
    override fun merge(other: Query<K>) =
        when (other) {
            ...
            is Before<K> -> if (key < other.key) Between(
                key,
                other.key
            ) else Never<K>()
            is Between<K> -> if (other(key)) Between(
                key,
                other.keyUpper
            ) else Never<K>()
        }
...
data class Before<K : Comparable<K>>(val key: K) : Query<K>() {
    ...
    override fun merge(other: Query<K>) =
        when (other) {
            ...
            is Never<K> -> Never<K>()
            is At<K> -> if (invoke(other.key)) other else Never<K>()
            is After<K> -> if (key > other.key) Between(
                other.key,
                key
            ) else Never<K>()
 ...
 * is greater than or equal to [keyLower] and less than [keyUpper].
 * @property keyLower The lower boundary, inclusive.
 * @property keyUpper The upper boundary, exclusive.
 */
data class Between<K : Comparable<K>>(
    ...
    override fun merge(other: Query<K>) =
        when (other) {
            ...
            is Never<K> -> Never<K>()
            is At<K> -> if (invoke(other.key)) other else Never<K>()
            is After<K> -> if (invoke(other.key)) Between(
                other.key,
                keyUpper
            ) else Never<K>()
            is Before<K> -> if (invoke(other.key)) Between(
                keyLower,
                other.key
            ) else Never<K>()
            is Between<K> -> {
 ...
 */
fun <K1 : Comparable<K1>, K2 : Comparable<K2>> split(query: Query<ComparablePair<K1, K2>>) =
    when (query) {
        is Always -> Always<K1>() to Always<K2>()
        is Never -> Never<K1>() to Never<K2>()
        is At -> At(query.key.first) to At(
            query.key.second
        )
        is After -> After(query.key.first) to After(
            query.key.second
        )
        is Before -> Before(query.key.first) to Before(
            query.key.second
        )
        is Between -> Between(
            query.keyLower.first,
            query.keyUpper.first
        ) to Between(
            query.keyLower.second,
            query.keyUpper.second
        )
    }